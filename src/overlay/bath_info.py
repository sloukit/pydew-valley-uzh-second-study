...
class BathInfo:
    def __init__(self):
        ...
        # Image pairs for different rounds - now using single images per round range
        # Since each image contains both "images" as mentioned in the issue
        self.image_pairs = {
            (7, 8): self.bath_images.get((7, 8), self.placeholder_image),  # rounds 7-8
            (9, 10): self.bath_images.get((9, 10), self.placeholder_image),  # rounds 9-10
            (11, 12): self.bath_images.get((11, 12), self.placeholder_image),  # rounds 11-12
        }
    ...
    def _load_placeholder_image(self):
        ...
        self.bath_images = {}
        try:
            self.bath_images[(7, 8)] = import_image("images/ui/graphs/de_bath_round_78.png")
            self.bath_images[(9, 10)] = import_image("images/ui/graphs/de_bath_round_910.png")
            self.bath_images[(11, 12)] = import_image("images/ui/graphs/de_bath_round_1112.png")
            
            # Use the first image to determine sizing
            first_image = self.bath_images[(7, 8)]
            ...
            for round_range, image in self.bath_images.items():
                if image.get_width() > max_width or image.get_height() > max_height:
                    ...
                    scale_factor = min(max_width / image.get_width(), max_height / image.get_height())
                    new_width = int(image.get_width() * scale_factor)
                    new_height = int(image.get_height() * scale_factor)
                    self.bath_images[round_range] = pygame.transform.scale(image, (new_width, new_height))
    ...
    def get_current_round_pair(self, current_round: int) -> tuple[pygame.Surface, pygame.Surface]:
